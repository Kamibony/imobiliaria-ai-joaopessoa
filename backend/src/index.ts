...
async function getConciergeSystemPrompt(brokerContext?: any) {
...
function convertZodJsonSchemaToVertexAiSchema(jsonSchema: any): any {
    if (!jsonSchema) return { type: FunctionDeclarationSchemaType.STRING };
    ...
    const resolveRef = (ref: string, defs: any) => {
    ...
    const convertNode = (node: any, defs: any): any => {
        ...
        if (schemaToProcess.anyOf) {
            ...
            const nonNullTypes = schemaToProcess.anyOf.filter((t: any) => t.type !== "null");
        ...
        if (schemaToProcess.type === "object" || (!schemaToProcess.type && schemaToProcess.properties)) {
            ...
            return {
             ...
             return {
                ...
                enum: schemaToProcess.enum?.filter((e:any)=> e !== null && e !== ""),
                description: schemaToProcess.description,
             };
...
const finalJsonSchema = zodToJsonSchema(FinalZodSchema as any, { target: "jsonSchema7" });
const finalSchema = convertZodJsonSchemaToVertexAiSchema(finalJsonSchema);
    ...
    if (validUnitsCount > 0) {
       try {
           ...
           const summary: any = {};
           if (min_area !== Infinity) summary.min_area_m2 = min_area;
           if (max_area !== -Infinity) summary.max_area_m2 = max_area;
           if (min_beds !== Infinity) summary.min_bedrooms = min_beds;
           if (min_price !== Infinity) summary.min_price_brl = min_price;
...
export const seedBrokers = onRequest(async (request, response) => {
  try {
    const demoBroker = {
